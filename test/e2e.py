...
async def main():
    with tempfile.TemporaryDirectory(prefix='town-e2e-') as directory:
        ...
        js = """import {startServer} from './src/server.mjs';
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                try:
                    ...
                    await page.wait_for_selector('.state-coding')
                    ...
                    print(json.dumps({'real_browser_server_e2e':True, 'installed_recorder_executed':True,
                        ...
                        'checks':['real CSP and ES modules','viewer authentication','synthetic provenance',
                            'two task isolation','ID-less approval resolution','privacy','client-side expiry','mobile layout']}, indent=2))
                finally:
                    await browser.close()
        finally:
